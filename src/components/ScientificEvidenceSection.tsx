...
export default function ScientificEvidenceSection() {
  ...
  // Localize all items according to active language (en/hi)
  ...
  const rightColumnItems = localizedItems.slice(5, 10);
  ...
  // Mobile auto-advance interval (when not paused/interacting)
  useEffect(() => {
    if (isMobilePaused) return;
    const timer = setInterval(() => {
      setMobileIndex((prev) => (prev + 1) % SCIENTIFIC_EVIDENCE_ITEMS.length);
    }, 3800);
  ...
  const handleTouchEnd = () => {
    if (touchStartX.current !== null && touchEndX.current !== null) {
      ...
      if (diff > 45) {
        ...
        // Swiped right -> Prev
        setMobileIndex((prev) => (prev - 1 + SCIENTIFIC_EVIDENCE_ITEMS.length) % SCIENTIFIC_EVIDENCE_ITEMS.length);
      }
    }
  ...
  const renderCard = (item: ScientificEvidenceItem, keyPrefix = "card") => (
    <div
      key={`${keyPrefix}-${item.id}`}
      className="group relative flex flex-col justify-between rounded-[24px] sm:rounded-[26px] bg-white/98 backdrop-blur-xl border border-pink-100/90 p-4 sm:p-5 shadow-[0_10px_30px_rgba(23,37,84,0.06)] hover:shadow-[0_20px_50px_rgba(234,52,132,0.18)] hover:border-[#F45B8A]/80 transition-all duration-300 overflow-hidden shrink-0 w-full h-[360px] sm:h-[380px]"
    >
      ...
      <div className="mt-3 pt-3 border-t border-pink-100/70 flex items-center gap-2">
        <Link
          ...
          className="flex-1 inline-flex items-center justify-center gap-1 rounded-xl bg-gradient-to-r from-[#172554] to-[#1e3a8a] px-2.5 py-2 text-[11px] font-semibold text-white hover:from-[#EA3484] hover:to-[#F45B8A] shadow-xs transition-all duration-300"
        >
  ...
  return (
    <section className="relative pt-3 sm:pt-6 pb-12 md:pb-16 bg-gradient-to-b from-[#FFF8FD] via-[#FFFDFE] to-[#FFF5F9] overflow-hidden border-b border-pink-100/70">
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 z-10">
        {/* ── DESKTOP: 3-COLUMN SHOWCASE (Matching User Reference Image with Side Auto Scroll) ── */}
        <div className="hidden lg:grid lg:grid-cols-12 gap-6 items-center min-h-[580px]">
          {/* 1. Left Column: Continuous Auto-Scroll Up Track */}
          <div className="lg:col-span-3 h-[580px] overflow-hidden relative rounded-3xl p-1">
            ...
            <div className="flex flex-col gap-4 animate-auto-scroll-up pause-on-hover">
              {leftScrollItems.map((item, idx) => (
                <div key={`left-${item.id}-${idx}`}>{renderCard(item, "left")}</div>
              ))}
            </div>
          </div>
            ...
            >
              <div className="rounded-2xl bg-white/90 border border-pink-100 p-2.5 shadow-xs">
                ...
                <span className="text-[10px] text-slate-500 font-medium">
                  {t("scientificEvidence.metrics.papers", "Clinical Papers")}
                </span>
              </div>
              <div className="rounded-2xl bg-white/90 border border-pink-100 p-2.5 shadow-xs">
                <span className="block text-base font-extrabold text-[#EA3484]">Level 1A</span>
                <span className="text-[10px] text-slate-500 font-medium">
                  {t("scientificEvidence.metrics.rcts", "Evidence RCTs")}
                </span>
              </div>
              <div className="rounded-2xl bg-white/90 border border-pink-100 p-2.5 shadow-xs">
                <span className="block text-base font-extrabold text-emerald-600">PubMed</span>
                <span className="text-[10px] text-slate-500 font-medium">
                  {t("scientificEvidence.metrics.indexed", "Indexed Trials")}
                </span>
              </div>
            </motion.div>
          ...
          <div className="lg:col-span-3 h-[580px] overflow-hidden relative rounded-3xl p-1">
            ...
            <div className="flex flex-col gap-4 animate-auto-scroll-down pause-on-hover">
              {rightScrollItems.map((item, idx) => (
                <div key={`right-${item.id}-${idx}`}>{renderCard(item, "right")}</div>
              ))}
            </div>
          </div>
        </div>

        {/* ── MOBILE & TABLET VIEW: CLEAN RESPONSIVE TOUCH CAROUSEL & AUTO-SLIDER ── */}
        <div className="lg:hidden space-y-6">
          {/* Header Block */}
          <div className="text-center space-y-2.5 px-2">
            <span className="inline-flex items-center gap-1.5 rounded-full border border-pink-200/90 bg-pink-50/90 px-3 py-1 text-[10.5px] font-bold uppercase tracking-wider text-[#EA3484]">
              <Microscope className="h-3 w-3 text-[#EA3484]" />
              <span>{t("scientificEvidence.badge", "Scientific Evidence & Clinical Trials")}</span>
            </span>

            <h2 className="text-xl sm:text-2xl font-extrabold text-[#172554] tracking-tight leading-snug">
              {t("scientificEvidence.heading", "Bridging Ancient Garbh Sanskar with")}{" "}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#EA3484] to-[#F45B8A]">
                {t("scientificEvidence.headingHighlight", "Modern Medical Science")}
              </span>
            ...
            <p className="text-xs text-[#475569] leading-relaxed max-w-sm mx-auto">
              {t(
                "scientificEvidence.subtitle",
                "Explore peer-reviewed medical studies, neuro-developmental research, and clinical validation."
              )}
            </p>
          </div>

          {/* Touch-Swipeable & Auto-Advancing Card Container */}
          <div
            className="relative px-2 max-w-sm sm:max-w-md mx-auto"
          ...
          >
            <div className="transition-all duration-500 ease-out">
              {renderCard(localizedItems[mobileIndex], `mobile-${mobileIndex}`)}
            </div>

            {/* Floating Next/Prev Arrow Buttons for Mobile */}
            <button
              onClick={() => {
                setIsMobilePaused(true);
                setMobileIndex((prev) => (prev - 1 + SCIENTIFIC_EVIDENCE_ITEMS.length) % SCIENTIFIC_EVIDENCE_ITEMS.length);
              ...
              className="absolute -left-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white/95 border border-pink-200/90 shadow-md flex items-center justify-center text-[#172554] hover:text-[#EA3484] z-20"
            ...
            <button
              ...
              className="absolute -right-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white/95 border border-pink-200/90 shadow-md flex items-center justify-center text-[#172554] hover:text-[#EA3484] z-20"
          ...
          <div className="flex items-center justify-center gap-1.5 pt-1">
            {SCIENTIFIC_EVIDENCE_ITEMS.map((_, i) => (
              <button
                key={i}
                onClick={() => {
                  setIsMobilePaused(true);
                ...
                className={`h-2 rounded-full transition-all duration-300 ${
                  i === mobileIndex ? "w-6 bg-gradient-to-r from-[#EA3484] to-[#F45B8A]" : "w-2 bg-pink-200 hover:bg-pink-300"
                }`}
          ...
          <div className="text-center pt-2">
            <Link
              to="/scientific-evidence"
              className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-[#EA3484] to-[#F45B8A] px-6 py-3 text-xs font-bold text-white shadow-md hover:shadow-lg active:scale-98 transition-all"
            >
