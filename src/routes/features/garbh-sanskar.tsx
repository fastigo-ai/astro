    ...
    <div className="min-h-screen bg-[#FFFCFE] text-[#475569] selection:bg-[#F45B8A]/20 selection:text-[#F45B8A]">
      ...
      <section id="plans-pricing" className="py-16 md:py-24 bg-[#FFFDFE] border-t border-pink-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {/* 3 Pricing Cards Grid - Compact with inner scroll */}
          <div className="grid lg:grid-cols-3 gap-6 lg:gap-7 items-stretch">
            {pricingPlans.map((plan) => {
              ...
              return (
                <div
                  key={plan.id}
                  className={`relative flex flex-col justify-between rounded-[26px] p-5 sm:p-6.5 transition-all duration-300 ${
                    isDiamond
                      ? "bg-gradient-to-b from-[#FFF6FA] via-white to-[#FDF4F8] border-2 border-[#F45B8A] shadow-[0_20px_50px_rgba(244,91,138,0.15)] lg:-translate-y-2"
                      : "bg-white border border-pink-100/90 shadow-[0_10px_30px_rgba(23,37,84,0.04)] hover:shadow-lg"
                  }`}
                >
                  {/* Most Popular Floating Top Pill */}
                  {plan.isPopular && (
                    <div className="absolute -top-3.5 left-1/2 -translate-x-1/2">
                      <span className="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full bg-gradient-to-r from-[#F45B8A] to-[#E91E63] text-white text-[11px] font-bold uppercase tracking-wider shadow-md">
                  ...
                  <div>
                    ...
                    <div className="flex items-center justify-between gap-2 mb-1.5">
                      <h3 className="text-xl sm:text-2xl font-bold text-[#172554] leading-snug">
                        {plan.name}
                      </h3>
                      {plan.id === "silver" && (
                    ...
                    <p className="text-xs text-slate-500 min-h-[32px] line-clamp-2 font-normal">
                      {plan.tagline}
                    </p>

                    {/* Price */}
                    <div className="mt-3.5 pb-4 border-b border-pink-100">
                      ...
                      <p className="text-[11px] text-[#F45B8A] font-semibold mt-0.5">
                    ...
                    <div className="mt-4 space-y-2">
                      ...
                      <div className="h-[190px] overflow-y-auto pr-1.5 space-y-1.5 rounded-xl border border-pink-100/70 bg-pink-50/20 p-2.5 custom-scrollbar">
                        {plan.features.map((item, fIdx) => (
                          <div
                            key={fIdx}
                            className="flex items-start gap-2 text-xs text-[#475569] leading-snug font-normal py-0.5"
                          >
                            <Check className="w-3.5 h-3.5 text-[#F45B8A] shrink-0 mt-0.5 font-bold" />
                            <span>{item}</span>
                          </div>
                        ))}
                      </div>
